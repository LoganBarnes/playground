...
var SpaceShip = function(spaceLib) {
	...
	// prim = spaceLib.addPrimitive(ShapeType.CYLINDER, transMat, [1.0, 1.0, 1.0, 1.0], [64.0, 0.0, 0.0, 1.0], "");
	// this.prims.push(new this.ShipPrim(prim, mat4.clone(transMat)));
...
SpaceShip.prototype.applyForce = function(force, point) {
	...
	// console.log(vec3.str(force) + ", " + vec3.str(point));
	...
	// // p = p + v * t
	// vec3.scale(vel, this.angVel, secs);
	// vec3.add(this.ang, this.ang, vel);

	// vec3.normalize(axis, this.ang);
	// angle = vec3.length(this.ang);

	// vec3.normalize(axis, this.ang);
	// angle = vec3.length(this.ang);

	// mat4.identity(this.rot);
	// mat4.rotate(this.rot, this.rot, degToRad(angle), axis);
	// mat4.invert(this.invRot, this.rot);
	
	// console.log("t: " + vec3.str(this.torque));
	// console.log("a: " + vec3.str(accel));
	// console.log("w: " + vec3.str(this.angVel));
	// console.log("v: " + vec3.str(vel));
	// console.log("p: " + vec3.str(this.ang));
	// console.log("axis: " + vec3.str(axis));
	// console.log("angle: " + angle);
	// console.log("rot: " + mat4.str(this.rot));
...
SpaceShip.prototype.update = function(secs, useGoalVel) {
...
SpaceLibGL.prototype.renderCubeMap = function(name) {
	...
	this.gl.uniformMatrix4fv(this.getUniform(cm.program, "uVMatrix"), false, this.spaceShip.getInvModel());
...
SpaceLibGL.prototype.handleKeyUp = function(keyEvent) {
	...
	switch(keyEvent.keyCode) {
		...
		case 32: // space
			...
			console.log(mat4.str(this.spaceShip.rot));
			break;
		case 77: // M
 ...
 * @Override
 */
SpaceLibGL.prototype.render = function() {
...
SpaceLibGL.prototype.tick = function() {
	...
	this.handleInput();
	this.spaceShip.update(deltaTime, !this.manualControls);
	this.render();
}
...
function main() {
	...
	// spaceLib.addOrbitCam(-0.0, 0.0, 15.0, [0, 0, 0], 45.0, 1.0, 0.1, 5000.0);
