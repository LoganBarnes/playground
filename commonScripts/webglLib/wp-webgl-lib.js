...
LibGL.prototype.initShapes = function() {
	sphere = new Sphere(50, 50, 1.0);
	cylinder = new Cylinder(50, 50, 1.0, 1.0);

	sphere.createVBO(this.gl, true, true);
	cylinder.createVBO(this.gl, true, true);
}
...
LibGL.prototype.addPrimitive = function(type, trans, color, settings, texture) {
	...
	this.primitives.push(new this.Primitive(type, mat4.clone(trans), invT, color, settings, texture));
 ...
 */
LibGL.prototype.renderPrimitives = function(program, callback, args) {
	...
	for (var i = 0; i < len; ++i) {
		...
		if (this.getTexture(prim.texture)) {
			this.gl.activeTexture(this.gl.TEXTURE0);
			this.gl.bindTexture(this.gl.TEXTURE_2D, this.getTexture(prim.texture));
			this.gl.uniform1i(this.getUniform(program, "uShapeTexture"), 0);
			// this.gl.uniform1i(this.getUniform(program, "uShadowTexture"), 0);
		}
		...
		switch(prim.type) {
			...
			case ShapeType.CYLINDER:
				cylinder.bindBuffer(this.gl, this.getAttribute(program, "aPosition"),
											 this.getAttribute(program, "aNormal"),
											 this.getAttribute(program, "aTexCoord"));
				cylinder.render(this.gl);
				break;
			case ShapeType.SPHERE:
				sphere.bindBuffer(this.gl,  this.getAttribute(program, "aPosition"),
											this.getAttribute(program, "aNormal"),
											this.getAttribute(program, "aTexCoord"));
				sphere.render(this.gl);
				break;
			default:
				break;
		}
 ...
 * Binds the given vbo.
 */
LibGL.prototype.bindBuffer = function(vbo, positionAttribute, normalAttribute, texCoordAttribute) {
	var step = Float32Array.BYTES_PER_ELEMENT;
	var stride = step * vbo.itemSize;
...
LibGL.prototype.renderModels = function(program, callback, args) {
	...
	for (var i = 0; i < len; ++i) {
		...
		// if the texture is used set set the uniform
		if (this.getTexture(model.texture)) {
			this.gl.activeTexture(this.gl.TEXTURE0);
 ...
 */
LibGL.prototype.handleSingleTexture = function(gl, texture) {
 ...
 */
LibGL.prototype.setUpTexture = function(initialArray, url) {
	// create texture and store the initialArray
	var texture = this.gl.createTexture();
	this.gl.bindTexture(this.gl.TEXTURE_2D, texture);
	this.gl.texImage2D(this.gl.TEXTURE_2D, 0, this.gl.RGB, 1, 1, 0, this.gl.RGB, this.gl.UNSIGNED_BYTE, initialArray);
	...
	texture.image.onload = function() {
		glLib.handleSingleTexture(glLib.gl, texture);
	}
...
LibGL.prototype.addTexture = function(name, initialArray, url) {
	this.textures[name] = this.setUpTexture(initialArray, url);
}
...
LibGL.prototype.resizeCanvas = function() {
	this.resize(this.canvas_div.offsetWidth, this.canvas_div.offsetWidth * 0.75);
	this.render();
}
...
LibGL.prototype.setShapeUniforms = function(sm, program, args) {
 ...
 */
LibGL.prototype.getRayBare = function(orig, scaleViewInv, canvas, mouseEvent) {

	var pos = this.getMousePos(mouseEvent);

	// convert mouse pos to screen space
	var x = pos.x * 2.0 / this.gl.canvas.width - 1.0;
	var y = 1.0 - pos.y * 2.0 / this.gl.canvas.height;
 ...
 */
LibGL.prototype.getRay = function(orig, scaleViewInv, canvas, mouseEvent) {
