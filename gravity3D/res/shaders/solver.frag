...
const float DENSITY = 50.0; // make uniform?
...
void main()
{
	...
	float radius = pow((3.0 * currPosition.w) / (DENSITY * 4.0 * PI), 1.0 / 3.0);
	...
	for (int r = 0; r < TEX_SIZE.y; r++)
	{
		for (int c = 0; c < TEX_SIZE.x; c++)
		{
			...
			// function of mass and density
			otherRadius = pow((3.0 * otherParticle.w) / (DENSITY * 4.0 * PI), 1.0 / 3.0);
